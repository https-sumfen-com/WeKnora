...
import { autoSetup, getCurrentUser, sumfenAutoLogin, userInfoFromApi } from '@/api/auth'
...
function persistLoginResponse(authStore: ReturnType<typeof useAuthStore>, response: any) {
  ...
  if (response.user && activeTenant && response.token) {
    authStore.setUser(userInfoFromApi(response.user, activeTenant.id))
...
router.beforeEach(async (to, from, next) => {
  ...
  // iframe 嵌入模式：hide_chat=1 时隐藏对话入口。
  // 只在参数明确出现时置 true，不在缺席时重置，确保 App.vue 清除参数后状态依然生效。
  if (to.query.hide_chat === '1') {
    uiStore.setHideChatMode(true)
  }
  ...
  if (to.meta.requiresAuth !== false) {
    // Sumfen 渠道自动登录：URL 携带渠道参数时始终覆盖当前会话（含已登录状态），
    // 支持 iframe 切换用户场景。App.vue 在挂载后会用 history.replaceState 清除
    // URL 参数，确保应用内导航不会重复触发此逻辑。
    if (hasPendingSumfenParams(to)) {
      ...
      if (result.success && result.loginData) {
        // 先清空当前会话再写入新用户数据，避免两套 token/user 混存
