...
用户直接给出唯一阀门组 ID 时，走独立的直接控制分支：不要求 `plot_id`，也不查询传感器、地块、天气或阀门关联；先展示该 ID 和用户可选提供的时长状态，后来一轮用户确认后再按 SONO 直接控制合同调用 `start_valve_bank`。`auto_off_minutes` 保持可选；用户未提供时长时不得臆造默认值，也不得在可信草稿或工具参数中发送该字段。
