...
- 每个**业务组织**（外部系统里的组织）对应一个 WeKnora **租户（Tenant）**，通过 `cid` 字符串标识
...
```
cid={cid}&mobile={mobile}&nonce={nonce}&ts={ts}
```
...
def build_iframe_url(base_url: str, cid: str, mobile: str, secret: str) -> str:
...
function buildIframeUrl(baseUrl, cid, mobile, secret) {
  const ts = Math.floor(Date.now() / 1000).toString();
  const nonce = crypto.randomBytes(16).toString('hex');
  const msg = `cid=${cid}&mobile=${mobile}&nonce=${nonce}&ts=${ts}`;
  const sig = crypto.createHmac('sha256', secret).update(msg).digest('hex');
  return `${baseUrl}/iframe-login?cid=${cid}&mobile=${mobile}&ts=${ts}&nonce=${nonce}&sig=${sig}`;
}
```
...
func BuildIframeURL(baseURL, cid, mobile, secret string) string {
    ...
    return fmt.Sprintf("%s/iframe-login?cid=%s&mobile=%s&ts=%s&nonce=%s&sig=%s",
        baseURL, cid, mobile, ts, nonce, sig)
}
```
...
| 现象 | 排查点 |
|---|---|
| 401 IFRAME_BAD_SIGNATURE，但 secret 确认没错 | 检查签名消息字典序是否正确（`cid < mobile < nonce < ts`）；检查字段值是否做了 URL 编码后再签（应该原样） |
