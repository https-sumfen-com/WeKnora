...
Commands read JSON from stdin and write JSON to stdout:
  build-panel   -> controlled form-panel goodsList + selected original rows
...
def build_panel(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "form_panel": {
            ...
            "autoOpen": payload.get("autoOpen", True),
...
def _frontend_submit_item(submitted: dict[str, Any], area: float) -> dict[str, Any]:
    total_num = _confirmed_quantity(submitted, area)
    return {
        "goods_name": _text(submitted.get("goods_name")).strip(),
        "stock_goods_id": _clean_number(submitted.get("stock_goods_id")),
        "is_formula": _clean_number(submitted.get("is_formula", 0)),
        "num": total_num,
        "price": _clean_number(submitted.get("price")),
...
def _merged_submit_item(
    ...
    return _frontend_submit_item(submitted, area)
