...
- Call `get_valve_bank_by_device` once for the whole comma-separated ID string. Treat the top-level returned valve-bank list as the authoritative server result; do not wrap it under `payload` or reconstruct it from nested device IDs.
...
- The frontend-visible irrigation confirmation must use only the dedicated `form irrigation-valve-duration` Syntax described in `references/irrigation-form.md`. If `build-panel` returns JSON, extract and send only the plain `form_syntax` string; never show the full JSON object or wrap the Syntax in JSON.
...
5. When irrigation is necessary, call `get_valve_bank_by_device` exactly once with `plot_device_ids` equal to the script-produced comma-separated string. Use the top-level returned valve-bank list directly as `valve_banks`, preserving its order.
...
7. Run `build-panel` with the complete `sensor_analysis`, evidence-backed reason, positive recommended duration, and returned valves. Preserve `pending_irrigation_draft` and send only its plain `form_syntax` string, which must begin with `form irrigation-valve-duration` and use `title 灌溉时长确认`, `autoOpen false`, `data`, and `valve_bank_title`. Do not return the `build-panel` JSON object. Read `references/irrigation-control.md` and `references/irrigation-form.md` first.
