---
name: sono-report
description: Generates plot-based SONO agricultural HTML reports from SONO-MCP business data, normalizes REPORT_DATA, renders sono-report/template.html, and saves reports under /app/report/. Use when the user asks for plot HTML reports, PDF-ready plot reports, plot weather/device/WOFOST sections, or saved agricultural operation reports.
---
...
本 Skill 只生成**基于地块的报告**。普通数据查询不要用本 Skill，仍按 `call-mcp-tools` 单工具问答处理。

工作流固定为：

```text
地块取数 → 构造 REPORT_DATA → 执行一体化脚本 → 返回最终 HTML 路径
```
...
- 只生成地块报告：报告内容必须围绕一个真实地块，不包含 `get_summary_base`、基地报告、企业报告或全局概览。
- 主数据源必须是 `get_plot_info`；天气、设备、WOFOST 只能作为该地块的可选章节。
...
- 具体字段规则：`../sono-mcp/references/tool-plot-info.md`、`../sono-mcp/references/tool-weather.md`、`../sono-mcp/references/tool-device-info.md`、`../sono-mcp/references/tool-wofost-report.md`
...
- `scripts/preprocess_report_data.py`：规范 `REPORT_DATA`，拒绝 `get_summary_base`。
