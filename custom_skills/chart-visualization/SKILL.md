...
生成图表时，**优先使用以下品牌色板**，通过 `style.palette` 字段传入。

### 主色 & 点睛色

| 色值 | 角色 | 使用场景 |
|------|------|----------|
| `#45C079` | **页面主色** | 使用频率最高，几乎任何场景都可使用 |
| `#FF934F` | **点睛色** | 危险预警（中级），也可用于数据图 |
| `#FECB00` | **点睛色** | 使用频率较少，可用于数据图 |
| `#BE225F` | **点睛色** | 危险级别最高的预警，也可用于数据图 |
| `#53BCFF` | **辅助色** | 应用场景广泛，与主色搭配使用（主色 70% / 辅助色 30%） |

**多系列图表默认色序**：`["#45C079", "#53BCFF", "#FF934F", "#FECB00", "#BE225F"]`

### 渐变色

渐变色**仅用于数据表（表格热力背景等），其他场景不使用**。
...
| 色值 | 角色 | 使用场景 |
|------|------|----------|
| `#0D0C08` | **深色主色** | 使用频率高，几乎任何场景都可使用 |
| `#FFFFFF` | **浅色主色** | 使用频率高，几乎任何场景都可使用 |
| `rgba(13,12,8,0.05)` | 底色 | 背景底色 |
| `rgba(13,12,8,0.10)` | 表格/按钮背景 | 表格行或按钮的背景色 |
| `rgba(13,12,8,0.15)` | 分割线 | 分隔线颜色 |
...
4. 瀑布图正负色建议：`["#45C079", "#BE225F", "#53BCFF"]`（正值、负值、汇总）
5. 非数据图场景不使用渐变色
