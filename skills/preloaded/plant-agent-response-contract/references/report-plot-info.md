...
**null 检查（先做）**：若 `plot_crop` 为 null 或 `plot_crop.crop` 为 null，则 作物/已生长天数/当前阶段/播种日期/预计收获 **全部跳过**，在 metric 卡片后写一行 Markdown：`"暂无种植批次数据，以下信息仅供参考。"`
...
若以上来源均无数据支撑 → 跳过 recommendation，Markdown 一句"暂无农事建议依据。"
