...
本参考只在用户明确要求生成、查看或输出地块报告时加载。普通地块情况、地块分析、地块基础信息或局部卡片展示，不加载本文件，使用通用 `card` / `chart` / `recommendation` 规则。

聚焦**基础种植状态 + 当前评级 + 农事建议**，不展示档案字段（地址、坐标、创建人、系统标识）。

明确地块报告的重点分析来源是 `get_wofost_report`。`get_plot_info` 只作为基础快照补充，用于名称、面积、当前作物、当前阶段、评级和建议依据；不要用它替代 WOFOST 的作物模型模拟分析。
