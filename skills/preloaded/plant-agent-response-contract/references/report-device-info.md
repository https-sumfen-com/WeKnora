...
本参考只在用户明确要求生成、查看或输出设备报告，或明确报告中需要设备维度时加载。普通设备状态、在线情况、最新遥测、设备异常关注或局部卡片展示，不加载本文件，使用通用 `card` / `chart` / `recommendation` 规则。
...
- `device_detail = null` → 跳过全部卡片，只生成补充设备详情或切换对象的 quick-reply
...
如果 `is_online=0`，必须生成 `recommendation`，并在 `reason` 中标注“设备当前离线，以下为最后上报数据”。
