...
本参考只在用户明确要求生成、查看或输出天气报告、天气预报报告、天气预警报告，或明确报告中需要天气维度时加载。

聚焦**预警判断 + 农事影响**，不逐字段罗列气象数值。

预警阈值和字段规则见 `sono-mcp/references/tool-weather.md`。

天气只有在用户明确要求天气报告、天气预报报告或天气预警报告时，才作为独立天气 report。若用户明确请求的是地块/基地/企业整体报告，天气是其中一个分析维度，必须合并进主 `report.cards[]`，不要生成第二份“天气报告”。

普通“天气趋势”“未来天气走势”“7天预报趋势”“降水/气温变化”等非报告问题不加载本文件，走 `card-weather.md`。
...
- 标题："未来7天天气预报"
- 图表优先：生成 ECharts `chart`，用日期为 x 轴，温度/降水/湿度/风力按数据语义选择 `line`、`bar` 或组合图
- 最高/最低温适合双折线或范围表达；降水适合柱状；湿度/风力可用折线或独立坐标轴
- 触发预警阈值的日期用 `markPoint`、`markLine`、标签或 `visualMap` 标注，不要默认做 table
- 日期格式：M月D日（如"1月15日"）
- 7条全部展示，不截断
- 如果用户明确要求逐日明细，再补充 table；默认报告主体是 chart
